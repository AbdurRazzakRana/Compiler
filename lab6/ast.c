...
Lab: 6
Updates:
  Modified PT method to add required space in front of the line
  in ASTPrint():
    Implemented all the cases in yacc and set appropritte next tree call
...
void ASTprint(int level, ASTnode *p) {
 ...
 switch (p->type) {    // Check the type of node
  case A_VARDEC:       // node is variable declaration type;
   PT(level);
   if(p->value == 0) printf("Variable %s %s\n", ASTtypeToString(p->my_data_type), p->name); // just a variable, printing the type and name
   else { // Array type declaration
    char pVal[30];
    sprintf(pVal, "%d", p->value);
    printf("Variable %s %s[%s]\n", ASTtypeToString(p->my_data_type), p->name, pVal);
   }
  ...
  case A_VARUSE:    // This case is for variable use such as x = 100.
   PT(level);
   printf("VARIABLE %s\n", p->name); // print the variable name
   if(p->value == 0)
    ASTprint(level, p ->s1); // variable list
   else {
    // array use
  ...
  case A_FUNCTIONDEC:    // case for function declaration, such as int func(){}
   ...
   ASTprint(level, p->next);    // Another funciton declaration with same level
   break;
  ...
  case A_PARAMS:                // case for function parameter
   ...
   if (p!= NULL && p->name != NULL){
    if(p->value != 0) printf("Parameter %s %s[]\n", ASTtypeToString(p->my_data_type), p->name); // for array parameter
    else printf("Parameter %s %s\n", ASTtypeToString(p->my_data_type), p->name);  // for variable parameter
