...
void emit_function(ASTnode *p, FILE *fp)
{
  ...
  parameterArrayCounter = 0;  // out of function scope, no need;
...
void emit_var(ASTnode *p, FILE *fp)
{
  ...
  {                                    // global variable
    ...
    if (p->s1 != NULL)
    { // addting additional offset for array
  ...
  { // local variable
    ...
    { // need to add additional offset for array
      if (parameterArrayCounter > 0)    // Need to check if the var is a param arrray
      {
        int isFound = 0, i;
...
void emit_params(ASTnode *p, FILE *fp, int tempReigsterNumber)
{
  ...
  if(p->value == -1){ // array as params
    strcpy(parameterArray[parameterArrayCounter], p->name);
    parameterArrayCounter = parameterArrayCounter + 1;
  }
  tempReigsterNumber++; // next argument will be in next t register
...
{ // As the p is A_RETURN_STAT box, it is not NULL
  ...
  if (p->s1 == NULL)
  { // return without any expression, return;
