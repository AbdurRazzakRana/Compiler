...
Lab: 9
Updates:
 Implemented all the functions from header file
*/
#include <stdio.h>
// #include <malloc.h>
#include <stdlib.h>
...
struct Func_Call * Insert_Func_Call(char *name,  int line_number)
{
 ...
 fc->next = NULL;
 if (first_Func_Call == NULL){ // when no items in the list
  first_Func_Call = fc;
  return fc;
 }
 ...
 while (p!=NULL) { // no need to insert if its alaready into the list
...
struct Func_Call * check_proto_gets_called(int totoalPrototypeLeft) {
 ...
 while(totoalPrototypeLeft--){
  if (callHead == NULL || protoHead == NULL) break;
  struct Func_Call * p = callHead;
  while(p != NULL){
   if((strcmp(p->name, protoHead->name)==0)){
    return p;
   }
   p = p->next;
  }
